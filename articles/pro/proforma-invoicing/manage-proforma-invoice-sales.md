...
description: מאמר זה מספק מידע על אופן העבודה עם חשבוניות פרופורמה של פרויקט.
author: rumant
ms.date: 04/05/2021
ms.topic: article
...
---
# <a name="manage-a-proforma-project-invoice"></a>ניהול חשבונית פרופורמה של פרוייקט 
...
עבור שורה **חשבונית זמן וחומר**, פרטי שורת החשבונית מקובצים לקטגוריות **לחיוב**, **לא לחיוב** ו **ללא תשלום** בדף **שורת חשבונית**. הפרטים של **שורת חשבונית הניתנת לחיוב** מסתכמים לסך הכל של שורת החשבונית. **‏‫ללא תשלום‬** ו **נתונים בפועל לא לחיוב** לא מתווספים לסכום שורת החשבונית.
...
| **בחר מוצר** | השדה מוגדר כברירת מחדל מהנתונים בפועל של מקור והוא שדה לקריאה בלבד. | בעת יצירת פרט חדש בשורת חשבונית ללא נתון בפועל לגיבוי, ניתן לערוך שדה זה. |
| **מוצר** | השדה מוגדר כברירת מחדל מהנתונים בפועל של מקור והוא שדה לקריאה בלבד. | בעת יצירת פרט חדש בשורת חשבונית ללא גיבוי בפועל, ניתן לערוך שדה זה אם השדה **בחר מוצר** מוגדר בתור **מוצר קיים**. |
| **שם המוצר** | השדה מוגדר כברירת מחדל מהנתונים בפועל של מקור והוא שדה לקריאה בלבד. | בפרט חדש בשורת חשבונית, שבו מזהה המוצר נבחר מקטלוג, שדה זה מוגדר לשם המוצר. עבור מוצר שאינו מופיע ברשימה‬, השדה מוגדר לשם שאינו מופיע ברשימה‬. |
| **תיאור שאינו מופיע ברשימה** | השדה מוגדר כברירת מחדל מהנתונים בפועל של מקור והוא שדה לקריאה בלבד. | בעת יצירת פרט חדש בשורת חשבונית ללא גיבוי בפועל, אתה יכול להוסיף תיאור שאינו מופיע ברשימה למוצר. |
...
| **מחלקת עסקאות** | מוגדר כברירת מחדל מהנתון בפועל מקור. שדה לקריאה בלבד שנעול לעריכה. | מוגדר כברירת מחדל אם המשתמש בוחר ליצור פרט בשורת החשבונית מסוג **זמן**, **הוצאה**, **חומר** או **עמלה** כאשר הוא יוצר גם **פרט שורת חשבונית** חדש ללא גיבוי בפועל. נעול לעריכה. |
