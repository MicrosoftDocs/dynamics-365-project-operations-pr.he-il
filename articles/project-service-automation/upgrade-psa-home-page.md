...
בעבר, לקוחות בעלי PSA גרסה ‎2.x שהיו צריכים לשדרג לגרסה ‎3.x, שהיא גרסת הממשק המאוחד מבוסס-לקוח (UCI) של PSA, היו חייבים לפנות לתמיכה של Microsoft ולספק את פרטי המופע שלהם כדי שהתמיכה תוכל לאפשר שידרוג לגרסה ‎3.x. החל מה-1 במרץ, 2020, לקוחות שלהם גרסה 2.x של PSA וצריכים לשדרג לגרסה 3.x, יוכלו לשדרג את המופעים שלהם ישירות מפורטל הניהול ללא צורך ביצירת קשר עם התמיכה של Microsoft.  
...
> [!NOTE]
> לקבלת החוויה הטובה ביותר בעת השימוש בגירסאות ישנות יותר של האפליקציות Field Service ו- PSA, עבור אל **הגדרות המערכת** ובשדה **‏‫השתמש בממשק המאוחד החדש בלבד (מומלץ)**, בחר **לא** הואיל וגירסאות אלה אינן מיועדות להיטען כראוי ב- UCI. לאחר הביטול של UCI, תוכל לפתוח ולהפעיל גירסאות אלה של Field Service ו- PSA באמצעות לקוח האינטרנט הישן. לקבלת הוראות לביטול לקוח UCI, ראה [הפעלת הממשק המאוחד בלבד](../admin/enable-unified-interface-only.md).
