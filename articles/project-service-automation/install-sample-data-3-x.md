...
4. אם אינך משתמש ב-Spencer Low עם שם משתמש **spencerl**, עליך לעדכן קובץ נוסף. פתח את הקובץ **DemoDataPreImportConfig.xml**, ולאחר מכן חפש את התג **userstocreateandconfigure**. עדכן את התג **\<כניסה\>** עם שם המשתמש של המשתמש שלך. לקבלת פרטים נוספים, ראה [הערות טכניות](#technical-notes).
...
- **\<EntitiesToEnableScheduling\>** מציין אילו ישויות צריכות להיות מופעלות עבור הזמנה ב- Microsoft Dynamics (נקרא גם Universal Resource Scheduling).
